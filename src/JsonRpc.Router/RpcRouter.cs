...
namespace JsonRpc.Router
{
	internal class RpcRouter : IRouter
	{
		...
		private List<RpcMethod> GetRpcMethods()
		{
			...
			foreach (var rpcClass in this.Configuration.Classes)
			{
				MethodInfo[] publicMethods = rpcClass.Value.GetMethods(BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly);
				foreach (MethodInfo publicMethod in publicMethods)
				{
					RpcMethod rpcMethod = new RpcMethod(rpcClass.Value, rpcClass.Key, publicMethod);
	...
	{
		internal Dictionary<string, Type> Classes { get; set; } = new Dictionary<string, Type>();
		public PathString RoutePrefix { get; set; }

		public void RegisterRpcGroup<T>(string groupName)
		{
			Type type = typeof(T);
			if (this.Classes.ContainsValue(type))
			{
				//TODO throw new RpcException("Type has already been registered");
			}
			if (this.Classes.ContainsKey(groupName))
			{
				//TODO throw new RpcException($"RPC group with name '{groupName}' has already been registered);
			}
			this.Classes.Add(groupName, type);
		}
	}
}